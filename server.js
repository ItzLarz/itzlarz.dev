...
var ipWhitelist = [null, "77.175.92.205"];
        ...
        if (data.includes(ip)) {
          if (!debugMode) {
            await client.sendMessage(
              ...
              "Client with ip " +
                ip +
                " accessed the server (old connection) (country: " +
          ...
          if (!debugMode) {
            await client.sendMessage(
              ...
              "Client with ip " +
                ip +
                " accessed the server (new connection) (country: " +
                geoip.lookup(ip).country +
                ")"
            );
            await client.markChatUnread(chatId);
...
app.get("/", async (req, res) => {
  ...
  if (useBlacklist) {
    for (var i = 0; i < ipBlacklist.length; i++) {
      if (!ip.startsWith(ipBlacklist[i])) {
        continue;
      } else {
        if (!debugMode) {
          await client.sendMessage(
            ...
            "Client with ip " +
              ip +
              " was kicked from the server (country: " +
              geoip.lookup(ip).country +
              ")"
          );
          await client.markChatUnread(chatId);
    ...
    if (!through) {
      ...
      res
        ...
        .send(
          "403 Forbidden" +
            `<br/>` +
            "IP-Adress not on the whitelist" +
            `<br/>` +
            new Date()
        );
      return res.end();
      ...
      if (geoip.lookup(ip).country != "NL") {
        ...
        res
          ...
          .send(
            "403 Forbidden" +
              `<br/>` +
              "Only Dutch IP-Adresses permitted" +
              `<br/>` +
              new Date()
          );
        return res.end();
  ...
  if (closed) {
    res
      .status(403)
      .send("403 Forbidden" + `<br/>` + "Site Closed" + `<br/>` + new Date());
    if (!debugMode) {
      await client.sendMessage(
        chatId,
        "Client with ip " +
...
if (!debugMode) {
  ...
  client.on("ready", async () => {
    ...
    server = https.createServer(serverOptions, app).listen(port, async () => {
      await client.sendMessage(
        chatId,
...
process.on("unhandledRejection", async (reason, origin) => {
  try {
    var errorFile = fs.openSync("./Errors.txt", "a");
    fs.appendFileSync(
      errorFile,
    ...
    console.log(`Rejection origin: ${origin}\n`);
  } finally {
    if (errorFile !== undefined) {
      fs.closeSync(errorFile);
    }
    ...
    try {
      ...
      client.initialize();
      client.on("ready", async () => {
        await client.sendMessage(chatId, "Whatsapp-webjs restarted");
      });
    }
  }
});
