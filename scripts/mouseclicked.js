function mouseReleased() {
	// Function called if mouse has been pressed
	let x = mouseX;
	let y = mouseY;
	let button;
	...
	if (!homeScreenState) {
		...
		if (
			button == "left" &&
			x > edgeSize + (boxSize * columns) / 2 - columns * 3.33 &&
			x < edgeSize + (boxSize * columns) / 2 + columns * 3.33 &&
			y > topBorderSize / 20 &&
			y < topBorderSize - topBorderSize / 20
		) {
			init();
		}
		...
		// Home button
			...
			button == "left" &&
			x > 0.33 * (2 * edgeSize + boxSize * columns) - 0.5 * buttonSize &&
			x < 0.33 * (2 * edgeSize + boxSize * columns) + 0.5 * buttonSize &&
			y >
		...
		// Music button
			...
			button == "left" &&
			x > 0.66 * (2 * edgeSize + boxSize * columns) - 0.5 * buttonSize &&
			x < 0.66 * (2 * edgeSize + boxSize * columns) + 0.5 * buttonSize &&
			y >
				(bottomBorderSize - edgeSize) / 2 +
					topBorderSize +
			...
			rect(
				...
				0.5 * bottomBorderSize + topBorderSize + 2 * edgeSize + boxSize * rows,
				buttonSize,
				buttonSize
			);
			if (musicState) {
